...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => HomePageBloc()..add(ShowListNoteEvent()),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: TextField(
                          ...
                          onChanged: (value) => context.read<HomePageBloc>().add(SearchNoteEvent()),
                          cursorHeight: 26,
  ...
  Widget itemBuilder(BuildContext context, int index) {
    ...
    var listNote = bloc.state.listNote;
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
                  listSearch.isEmpty
                      ? (listNote.indexOf(item) % 2 == 0
                          ? '\t${item.titleNote}\n'.trim()
                          : '\t${item.titleNote} ${item.dateTime.substring(0, 5)}\n'.trim())
                      : (listSearch.indexOf(item) % 2 == 0
                          ? '\t${item.titleNote}\n'.trim()
                          : '\t${item.titleNote} ${item.dateTime.substring(0, 5)}\n'.trim()),
              ...
              Padding(
                ...
                child: Text(
                  listSearch.isEmpty
                      ? (listNote.indexOf(item) % 2 == 0
                          ? (item.detailsNote.length <= 116
                              ? '${item.dateTime} ${item.detailsNote}'
                              : '${'${item.dateTime} ${item.detailsNote}'.substring(0, 116 - item.dateTime.length)}...')
                          : (item.detailsNote.length <= 116
                              ? item.detailsNote
                              : '${item.detailsNote.substring(0, 116)}...'))
                      : (listSearch.indexOf(item) % 2 == 0
                          ? (item.detailsNote.length <= 116
                              ? '${item.dateTime} ${item.detailsNote}'
                              : '${'${item.dateTime} ${item.detailsNote}'.substring(0, 116 - item.dateTime.length)}...')
